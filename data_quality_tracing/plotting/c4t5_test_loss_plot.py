...
@dataclass
class Cfg:
    # Folder containing saved C4+T5 run_*.json files
    source_results_dir: str = "../output/eval_exp3_final_1"

    # Folder to write all new comparison figures/tables
    out_dir: str = "../output/eval_exp3_final_replot"
    ...
    inset_fixed_ylim: Optional[Tuple[float, float]] = None
    inset_y_pad_frac: float = 0.05
...
def write_curves_csv(path: Path, runs: List[Dict[str, Any]]):
    fields = ["run_name", "step", "dev_loss", "test_loss", "wall_sec"]
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
...
def plot_test_loss_figure(
    ...
    for r in runs:
        pts = r["curve"]
        pts = [p for p in pts if int(p["step"]) <= 120]
...
def main():
    ...
    for fname in cfg.run_files:
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                f"Set source_results_dir in Cfg to the folder containing your C4+T5 run_*.json files."
            )
        run = load_json(path)
        run_name = str(run.get("run_name", ""))
        if "random" in run_name.lower() or "random" in path.name.lower():
            continue
        runs.append(run)
    ...
    print(f"[DONE] Replotted from saved C4+T5 runs in: {source_dir}")
    ...
    print()
    print("[NOTE] The random group has been removed completely.")

...
@dataclass
class Cfg:
    # Folder containing saved TinyStories run_*.json files
    source_results_dir: str = "../output/eval_tinystories33m_test_loss"

    # Folder to write all new comparison figures/tables
    out_dir: str = "../output/eval_tinystories33m_replot"
...
def main():
    ...
    for fname in cfg.run_files:
        ...
        if not path.exists():
            raise FileNotFoundError(
                ...
                f"Set source_results_dir in Cfg to the folder containing your TinyStories run_*.json files."
            )
        run = load_json(path)
    ...
    print(f"[DONE] Replotted from saved TinyStories runs in: {source_dir}")

#!/usr/bin/env python3
"""
merge_inrun_fidelity_shards_tinystories33m.py

Merge TinyStories-33M shard outputs and build pooled fidelity plots.
...
def main() -> None:
    ...
    points_out = out_dir / f"fidelity_multibatch_points_tinystories33m{suffix}.csv"
    batches_out = out_dir / f"fidelity_multibatch_batch_summary_tinystories33m{suffix}.csv"
    ...
    y = np.array([float(r["phi_est_duoshap"]) for r in all_points], dtype=np.float64)
    ...
    summary_path = out_dir / f"fidelity_multibatch_summary_tinystories33m{suffix}.json"
    summary_path.write_text(json.dumps(summary, indent=2))

    plot_path = out_dir / f"fidelity_multibatch_scatter_tinystories33m_final{suffix}.pdf"

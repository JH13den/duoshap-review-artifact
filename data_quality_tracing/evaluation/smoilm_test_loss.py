...
class Cfg:
    ...
    # DuoShap output written by your SmolLM in-run script
    shapley_dir: str = "../output/smollm135m_soda_run"
    shapley_csv: str = "soda_inrun_shapley_values.csv"

    # Output
    out_dir: str = "../output/eval_smollm135m_test_loss"

    # Smaller universe to match the previous convergence-comparison logic
    train_universe: int = 5000
...
def write_caption_file(path: Path, cfg: Cfg) -> None:
    caption = (
        "Test loss versus optimization steps for SmolLM-135M retrained on filtered "
        "SmolLM subsets. We compare the original training set (Original-100%) with "
...
def train_run(
    ...
    data_cfg = DataConfig()
    _, model = load_smollm_135m(data_cfg)
...
def main():
    ...
    # -------------------------
    data_cfg = DataConfig()
    _, init_model = load_smollm_135m(data_cfg)
    ...
    # Plots
    # -------------------------
    title_main = "SmolLM-135M Convergence on Filtered SmolLM Subsets"
    subtitle_main = (
    ...
    plot_train_probe_loss_vs_step(
        ...
        runs,
        cfg,
        title="SmolLM-135M Optimization Behavior on a Fixed Train-Probe Subset",

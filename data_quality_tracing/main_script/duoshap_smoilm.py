...
class SODAInRunConfig:
    # Where load_model_data_smollm135m.py wrote the subsets
    ...
    # Output
    output_dir: str = "../output/smollm135m_soda_run"
...
# ---------------------------------------------------------------------
# Dataset prep: load fixed blocked SmolLM subsets
# ---------------------------------------------------------------------
...
def main():
    ...
    _ = tokenizer  # kept for symmetry with previous scripts
    model.to(device)

    # Data
    train_loader, val_loader, val_inputs = load_blocked_smollm_subsets(cfg, device)

#!/usr/bin/env python3
"""
eval_inrun_fidelity_multibatch_shard_smollm135m.py

Multi-batch fidelity check for SmolLM-135M / blocked SmolLM dataset.

Main logic:
- Keep EACH batch as its own local Shapley game.
- For each batch:
    (A) compute MC permutation Shapley on the SAME local utility U(S)
    (B) compute DuoShap / tabular antithetic estimate on the SAME U(S)
...
from load_model_smoilm import Config as DataConfig, load_smollm_135m
...
class FidelityEvalConfig:
    output_dir: str = "../output/eval_exp2_fidelity_multibatch_smollm135m"
    seed: int = 42

    # Data written by your SmolLM setup script
    data_output_dir: str = "../output/smollm135m_setup"
    train_subdir: str = "smollm_train_blocks"
    val_subdir: str = "smollm_val_blocks"
    ...
    max_val_for_local_utility: int = 128
    ...
    # Plot formatting (optional shard-level plots only)
    force_sci_exp: int = -3
...
# Dataset loading
# -----------------------------
def load_blocked_smollm_subsets(
    ...
    if cfg.randomize_val_for_utility:
        rng = np.random.RandomState(cfg.seed + cfg.val_utility_seed_offset)
    ...
    return train_loader, val_loader, val_inputs_default
...
def simulate_one_subset_update(
    ...
    """
    Deterministic virtual-update utility:
...
def estimate_phi_duoshap_tabular_antithetic_per_i(U: UtilityCache, N_per_i: int, seed: int) -> np.ndarray:
    """
    Tabular DuoShap estimator with per-i antithetic grouping.
...
def plot_scatter_paper_like(
    ...
    ax.set_title(
        f"SmolLM-135M DuoShap vs MC ({n_batches} batches, {title_suffix})\n"
...
def parse_args() -> FidelityEvalConfig:
    ...
    parser.add_argument("--output_dir", type=str, default="../output/eval_exp2_fidelity_multibatch_smollm135m")
    parser.add_argument("--seed", type=int, default=42)

    parser.add_argument("--data_output_dir", type=str, default="../output/smollm135m_setup")
    parser.add_argument("--train_subdir", type=str, default="smollm_train_blocks")
    parser.add_argument("--val_subdir", type=str, default="smollm_val_blocks")
    ...
    parser.add_argument("--max_val_for_local_utility", type=int, default=128)
    ...
    parser.add_argument("--N_per_i", type=int, default=6)
...
def main() -> None:
    ...
    # Load model/data once per shard
    data_cfg = DataConfig()
    tokenizer, model = load_smollm_135m(data_cfg)
    _ = tokenizer
    model.to(device)
    ...
    train_loader, val_loader, val_inputs_default = load_blocked_smollm_subsets(E, device)
    ...
    for global_batch_id in batch_ids:
        ...
        phi_est = estimate_phi_duoshap_tabular_antithetic_per_i(U, N_per_i=E.N_per_i, seed=duoshap_seed)
        ...
        for i in range(B):
            point_rows.append({
                ...
                "phi_est_duoshap": float(phi_est[i]),
    ...
    print("[DONE] SmolLM multi-batch shard finished successfully.")

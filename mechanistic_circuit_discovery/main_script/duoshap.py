...
Unified Global SODA (Eq. 3) runner (GT/IOI semantics) for the unified cache layout:
  ...
  ../cache_output/{TASK_KEY}/{MODEL_KEY}/nodes/{split}/batch_{b}/
    head_L{L}_H{h}.pt   [B, T, d_model]  (post-W_O head writes)
    mlp_L{L}.pt         [B, T, d_model]
    batch_index.json

Guarantees (matches your old GT/IOI scripts):
- Patch ONLY last position
- Patch points:
    blocks.{L}.attn.hook_result   (expects [B,T,d_model])
...
IMPORTANT TLens COMPAT:
Some TransformerLens versions don't accept attention_mask in forward/run_with_hooks.
This script auto-falls back if unsupported.
...
CONFIG = {
    ...
    "TASKS": {
        ...
        "docstring": {
            ...
            # Keep your existing pipeline; only evaluation differs (adds logprob/logit-diff)
            "task_type": "docstring",
            "seed": 123,
        },
        # You can add more tasks later as long as you cache them to the same layout.
    },
}
# ==============================================================================
...
def tl_call(model: HookedTransformer, input_ids: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
    """Compat: some TransformerLens versions don't accept attention_mask."""
    try:
...
def tl_call_with_hooks(
    ...
    """Compat: some TransformerLens versions don't accept attention_mask in run_with_hooks."""
    try:
...
def mix_lastpos_heads(last_pos, keep_clean_nodes, heads_clean, heads_corr):
    ...
    lp_idx = torch.as_tensor(last_pos, device=device)
...
def make_overwrite_lastpos_hook(last_pos, repl_rows_Bd):
    ...
    def _hook(act, hook):
        ...
        if idxs is None or idxs.device != act.device:
            idxs = torch.as_tensor(last_pos, device=act.device)
...
def dataset_value_global(
    ...
    for bidx, s, e in batches(N, bs):
        ...
        tok_batch = tok(prompts, return_tensors="pt", padding=True)
...
def evaluate_ioi(
    ...
    def _core(per_layer_keep: Dict[int, Set[int]], *, debug=False) -> Dict[str, Any]:
        total_kl, total_B = 0.0, 0
        sum_logprob_io, sum_margin = 0.0, 0.0
        preds_io = preds_s = preds_other = 0
        ...
        for bidx, s, e in batches(len(clean_recs), bs):
            ...
            tgt_ids, dis_ids = get_batch_ids_ioi(recs_batch, tok)
            tgt_ids = tgt_ids.to(device); dis_ids = dis_ids.to(device)
            ...
            pos = lq[torch.arange(B, device=device), tgt_ids]
            neg = lq[torch.arange(B, device=device), dis_ids]
            margin = pos - neg
            io_logprob = logp_patch[torch.arange(B, device=device), tgt_ids]

            sum_logprob_io += io_logprob.mean().item() * B
            sum_margin     += margin.mean().item() * B

            preds = lq.argmax(dim=-1)
            preds_io    += int((preds == tgt_ids).sum().item())
            preds_s     += int((preds == dis_ids).sum().item())
            preds_other += int(B - ((preds == tgt_ids) | (preds == dis_ids)).sum().item())

            total_B += B

            del logits_clean, logits_patched, lp, lq, logp_clean, logp_patch, p_clean, kl, io_logprob, pos, neg, margin
            if device.type == "cuda":
                torch.cuda.empty_cache()

        neg_kl = - (total_kl / total_B)
        mean_logprob_io = (sum_logprob_io / total_B)
        mean_margin = (sum_margin / total_B)
        return {
            "neg_kl": neg_kl,
            "kl": -neg_kl,
            "mean_logprob_target": mean_logprob_io,
            "cross_entropy": -mean_logprob_io,
            "mean_logit_diff": mean_margin,
            "pred_frac_IO": preds_io / max(1, total_B),
            "pred_frac_S": preds_s / max(1, total_B),
            "pred_frac_OTHER": preds_other / max(1, total_B),
            "n_examples": total_B,
        }
...
def evaluate_docstring_add_metrics(
    ...
    """
    Keeps the SAME KL semantics as evaluate_generic_lastpos,
    but adds GT/IOI-style keys:
      - mean_logprob_target
      - cross_entropy
      - mean_logit_diff

    Uses your docstring jsonl fields (as in your example):
      correct_token_ids: [id]   (target id = first element)
      wrong_token_ids:   [id...]
    Define distractor per example as the hardest wrong: argmax logit among wrong_token_ids.
    mean_logit_diff = logit(correct) - max_logit(wrong candidates)
    """
    n_layers = model.cfg.n_layers
    all_layers = list(range(n_layers))
    ...
    def _core(per_layer_keep: Dict[int, Set[int]]) -> Dict[str, Any]:
        ...
        for bidx, s, e in batches(len(clean_recs), bs):
            ...
            # --- target + wrong candidates (from your dataset) ---
            ...
            # --- KL(clean || patched) (same as your generic_lastpos) ---
            logp = F.log_softmax(lp, dim=-1)
            logq = F.log_softmax(lq, dim=-1)
            p = logp.exp()
...
def run_task(model: HookedTransformer, tok, task_key: str, spec: Dict[str, Any]) -> None:
    ...
    if spec["task_type"] == "gt":
        ...
        # ONLY change for docstring: add extra metrics, keep KL semantics
